...
def calculate_concrete_capacity(concrete_grade, beam_width, effective_depth, rebar_size=0, rebar_spacing=0):
    ...
    (Note: This version uses only the available inputs from the form.)
    """
    # Example: Use 32 for C32/40 and 40 for C40/50
    fck = 32 if concrete_grade == "C32/40" else 40
    # In a more detailed design, you'd include reinforcement parameters (rebar_size, rebar_spacing)
...
def calculate_applied_loads(span_length, loading_type, additional_loads):
    ...
    # Default load values based on loading type
    if loading_type == "HA":
        udl = 30  # kN/m
        point_load = 120  # kN
    ...
    logging.debug("Default loads: UDL=%s, Point Load=%s", udl, point_load)
...
def calculate_beam_capacity(form_data, loads):
    """
    Reads input parameters, calculates beam capacities, applied loads and checks the utilisation.
    """
    ...
    if material == "Steel":
        ...
        # (Rebar details are not in the current form; using default zeros)
